...
module Rfm
  module Factory # :nodoc: all
    class DbFactory < Rfm::CaseInsensitiveHash
      ...
      def all
        if !@loaded
          Rfm::Resultset.new(@server, @server.connect(@server.state[:account_name], @server.state[:password], '-dbnames', {}).body).each {|record|
    ...
    class LayoutFactory < Rfm::CaseInsensitiveHash
      ...
      def all
        if !@loaded
          Rfm::Resultset.new(@server, @server.connect(@server.state[:account_name], @server.state[:password], '-layoutnames', {"-db" => @database.name}).body).each {|record|
    ...
    class ScriptFactory < Rfm::CaseInsensitiveHash
      ...
      def all
        if !@loaded
          Rfm::Resultset.new(@server, @server.connect(@server.state[:account_name], @server.state[:password], '-scriptnames', {"-db" => @database.name}).body).each {|record|

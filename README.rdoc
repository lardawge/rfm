...
Rfm 1.0.0 is broken and does not work as advertised. Second it has a few bug fixes that were never pushed to rfm 1.0.0. Last but not least is that the xml parser is
switched to nokogiri which is a huge speed improvement over REXML (used in rfm 1.0.0) on large datasets.

Rfm was originally designed by Six Fried Rice co-founder Geoff Coffey.
...
- Larry Sprock added ssl support and switched the xml parser to a much faster Nokogiri and is at this point the primary maintainer. Since 1.0 it has gone through some major rewrites and refactoring in an effort to make it lean and mean... also to satisfy my own coding curiosities.
...
IMPORTANT:SSL and Certificate verification are on by default. Please see Rfm::Server for explanation and setup.
...
Finally, if you want to introspect the server and find out what databases are available, you can do this:
  ...
  all_dbs = my_server.databases

In any case, you get back Rfm::Database objects. A database object in turn has a layout method.

  my_layout = my_db.layout("My Layout")

And to get them all:

  all_layouts = my_db.layouts

Bringing it all together, you can do this to go straight from a server to a specific layout:

  my_layout = my_server.db("My Database").layout("My Layout")

NOTE: for the ruby experts, Rfm::Server#db is an Rfm::DatabaseFactory object, which is just a fancied up Hash. So anything you can do with a hash, you can also do with my_server.db. The same goes for my_db.layouts, which is an Rfm::LayoutFactory.
...
For a complete list of the available options, see the "expand_options" method in the Rfm::Server.
...
FileMaker's number field is insanely robust. The only data type in ruby that can handle the same magnitude and precision of a FileMaker number is Ruby's BigDecimal. (This is an extension class, so you have to require 'bigdecimal' to use it yourself). Unfortunately, BigDecimal is not a "normal" ruby numeric class, so it might be really annoying that your tiny filemaker numbers have to go this route. This is a great topic for debate.
...
There are two methods to help track down problems. When you create a server object, you can provide two additional optional parameters:
